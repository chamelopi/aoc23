...
use crate::errors::AocErrors;

static INPUT: &str = "Game 1: 3 blue, 7 green, 10 red; 4 green, 4 red; 1 green, 7 blue, 5 red; 8 blue, 10 red; 7 blue, 19 red, 1 green
...
type Configuration = HashMap<Color, u32>;

struct Game {
    id: u32,
    cubes_shown: Vec<Configuration>,
}
...
fn sum_of_possible_ids(input: &str) -> u32 {
    let config = HashMap::from([(Color::Red, 12), (Color::Green, 13), (Color::Blue, 14)]);

    let games: Vec<Game> = parse_games(input).expect("could not parse games!");
    games
        .iter()
...
fn parse_games(input: &str) -> Result<Vec<Game>> {
    let game_id_regex = Regex::new("Game (?<id>\\d+)")?;
    let color_regex = Regex::new("(?<count>\\d+) (?<color>red|green|blue)")?;
    let mut games = vec![];

    for line in input.lines() {
        let prefix_and_output = line.split(":").map(|e| e.to_owned()).collect::<Vec<_>>();
        let game_id_match = game_id_regex
            .captures(&prefix_and_output[0])
            .ok_or(AocErrors::ParseError("Cannot parse game id".into()))?;
        let game_id = game_id_match["id"].parse::<u32>()?;
        let mut cubes_shown = vec![];

        for draws in prefix_and_output[1].split(";") {
            let mut config: Configuration = HashMap::new();
            
            for draw in draws.split(",") {
                let matches = color_regex.captures(draw).ok_or(AocErrors::ParseError("Cannot parse color draw".into()))?;
                config.insert(matches["color"].into(), matches["count"].parse::<u32>()?);
            }

            cubes_shown.push(config);
        }

        games.push(Game { id: game_id, cubes_shown });
    }
