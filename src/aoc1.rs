use core::panic;
use std::{vec::Vec, ops::Deref};

static INPUT: &'static str = "8eight1
98126
...
fn first_and_last(line: &str) -> (u32, u32) {
    ...
    for chr in line.chars() {
        ...
        if first.is_none() {
            first = chr.to_digit(10);
        } else {
            last = chr.to_digit(10);
        }
    }
    
    (first.unwrap(), last.or(first).unwrap())
}

fn first_and_last_with_str(line: &str) -> (u32, u32) {
    let mut my_findings = vec![];

    for (_, digit) in DIGITS_STRING.iter().enumerate() {
        let matches = line.match_indices(digit).collect::<Vec<_>>();

        if !matches.is_empty() {
            // Add first and last match
            my_findings.push(matches[0]);
            my_findings.push(matches[matches.len()-1]);
        }
    }

    my_findings.sort_by(|a, b| a.0.partial_cmp(&b.0).unwrap());

    parse_digits(my_findings[0], my_findings[my_findings.len()-1])
}

fn parse_digits(first: (usize, &str), last: (usize, &str)) -> (u32, u32) {
    (parse_digit(first), parse_digit(last))
}

fn parse_digit(pair: (usize, &str)) -> u32 {
    // Not a digit
    if pair.1.len() > 1 {
        for (i, digit_word) in DIGITS_STRING.iter().enumerate() {
            if digit_word.deref() == pair.1 {
                return i as u32 + 1;
            }
        }
        panic!("this should not happen!")
    } else {
        pair.1.chars().next().unwrap().to_digit(10).unwrap()
    }
}

fn calculate_sum(input: &str) -> u32 {
    input.lines().map(first_and_last)
        .map(|(first, last)| (first * 10 + last))
        .sum::<u32>()
}

fn calculate_sum_second(input: &str) -> u32 {
    input.lines().map(first_and_last_with_str)
        .map(|(first, last)| (first * 10 + last))
        .sum::<u32>()
}
...
#[cfg(test)]
mod tests {
    use crate::aoc1::{calculate_sum, calculate_sum_second};
    ...
    fn it_works() {
        ...
        assert_eq!(142, calculate_sum(test_input));
    }
    ...
    fn second_part_works() {
        ...
        assert_eq!(281, calculate_sum_second(test_input));
    }
}
...
pub fn aoc1() {
    ...
    let sum2 = calculate_sum_second(INPUT);
    println!("Second sum is {}", sum2);
}
